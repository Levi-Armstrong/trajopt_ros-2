...
public:
  ...
  public:
    ...
    bool operator!=(const Point32& b) const { return (x != b.x) || (y != b.y) || (z != b.z); }
    bool isZero() { return (x == 0) && (y == 0) && (z == 0); }
    Point64 cross(const Point32& b) const { return Point64(y * b.z - z * b.y, z * b.x - x * b.z, x * b.y - y * b.x); }
    Point64 cross(const Point64& b) const { return Point64(y * b.z - z * b.y, z * b.x - x * b.z, x * b.y - y * b.x); }
  ...
  public:
    ...
    Int128 operator-() const { return Int128((uint64_t) - (int64_t)low, ~high + (low == 0)); }
    ...
    btScalar toScalar() const
    {
      return ((int64_t)high >= 0) ?
                 btScalar(high) * (btScalar(0x100000000LL) * btScalar(0x100000000LL)) + btScalar(low) :
                 -(-*this).toScalar();
    }

    int32_t getSign() const { return ((int64_t)high < 0) ? -1 : (high || low) ? 1 : 0; }
  ...
  public:
    ...
    btScalar xvalue() const { return (point.index >= 0) ? btScalar(point.x) : point128.xvalue(); }
    btScalar yvalue() const { return (point.index >= 0) ? btScalar(point.y) : point128.yvalue(); }
    btScalar zvalue() const { return (point.index >= 0) ? btScalar(point.z) : point128.zvalue(); }
  ...
  public:
    ...
    void print()
    {
      printf("E%p : %d -> %d,  n=%p p=%p   (0 %d\t%d\t%d) -> (%d %d %d)",
             this,
  ...
  private:
    ...
    static uint64_t mul(uint32_t a, uint32_t b) { return (uint64_t)a * (uint64_t)b; }
...
btConvexHullInternal::Orientation
btConvexHullInternal::getOrientation(const Edge* prev, const Edge* next, const Point32& s, const Point32& t)
{
      ...
      if (e1 && (e1->target != stop1))
      {
        ...
        if (f1->copy > mergeStamp)
        {
          ...
          if (d1.dot(normal) == 0)
          {
            ...
            if ((dxn > 0) &&
                ((dx1 == 0) ? (dy1 < 0) : ((dx1 < 0) && (Rational64(dy1, dx1).compare(Rational64(dy, dx)) > 0))))
            {
              e1 = f1;
              et1 = e1->target->point;
              dx = dxn;
      ...
      if (e0 && (e0->target != stop0))
      {
        ...
        if (f0->copy > mergeStamp)
        {
          ...
          if (d0.dot(normal) == 0)
          {
            ...
            if ((dxn < 0) &&
                ((dx0 == 0) ? (dy0 > 0) : ((dx0 < 0) && (Rational64(dy0, dx0).compare(Rational64(dy, dx)) < 0))))
            {
              e0 = f0;
              et0 = e0->target->point;
              dx = dxn;
  ...
  while (true)
  {
    ...
    if (cmp > 0)
    {
      ...
      v->point128 = PointR128(
          Int128::mul(face->dir0.x * r0, m11) - Int128::mul(face->dir0.x * r1, m01) +
...
static int32_t getVertexCopy(btConvexHullInternal::Vertex* vertex,

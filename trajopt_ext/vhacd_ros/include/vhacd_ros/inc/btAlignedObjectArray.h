...
/// It is developed to replace stl::vector to avoid portability issues, including STL alignment issues to add SIMD/SSE
/// data
template <typename T>
// template <class T>
...
protected:
  SIMD_FORCE_INLINE int32_t allocSize(int32_t size) { return (size ? size * 2 : 1); }
...
public:
  ...
  /// Generally it is best to avoid using the copy constructor of an btAlignedObjectArray, and use a (const) reference
  ...
  SIMD_FORCE_INLINE int32_t size() const { return m_size; }
  ...
  /// clear the array, deallocated memory. Generally it is better to use array.resize(0), to reduce performance overhead
  /// of run-time memory (de)allocations.
  ...
  /// when the new number of elements is smaller, the destructor will be called, but memory will not be freed, to reduce
  /// performance overhead of run-time memory (de)allocations.
  ...
  /// return the pre-allocated (reserved) elements, this is at least as large as the total number of elements,see size()
  /// and reserve()
  SIMD_FORCE_INLINE int32_t capacity() const { return m_capacity; }
